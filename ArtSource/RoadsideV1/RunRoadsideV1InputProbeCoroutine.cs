...
var keyboard=UnityEngine.InputSystem.InputSystem.AddDevice<UnityEngine.InputSystem.Keyboard>("RoadsideV1CoroutineKeyboard");
...
System.Collections.IEnumerator Probe(){
 ...
 try{
  ...
  System.IO.File.WriteAllText("Artifacts/RoadsideV1/RoadsideV1_InputProbe_Attempt2.json",report,new System.Text.UTF8Encoding(false));
  if(!passed)UnityEngine.Debug.LogError("RoadsideV1 coroutine input probe failed: "+report);else UnityEngine.Debug.Log("RoadsideV1 coroutine input probe passed: "+report);
 }finally{cleanup();}
}
controller.StartCoroutine(Probe());
System.IO.File.WriteAllText("Artifacts/RoadsideV1/RoadsideV1_InputProbe_Attempt2.json","{\"status\":\"running\"}",new System.Text.UTF8Encoding(false));
}))();
