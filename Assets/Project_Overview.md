...
6. **Bridge Assembly & Foundation Engineering**: Players mount structural members into designated sockets (`BridgeMountSocket`, `BridgeConstructionSite`). Abutments and main girders use independent signed length/width leveling values: four locally marked hammer points adjust the supports, four Spirit Level positions measure either axis from both sides, and a separate server-validated confirmation advances the workflow. Other sub-assembly steps include diagonal fastener pairs, concrete preparation and pouring, and tool-driven fastening.
...
- `PlayerSpiritLevelController`: Owner-input/server-validation controller that replicates one of four exact measurement points while each client derives its axis and renders the vial reading and center marks locally.
