...
public class MountableBridgeComponent : NetworkBehaviour, IPIckableNew, IInteractableNew, ISharedCarryObject, IHeldObjectHudInfoProvider, ICarriedObjectImpactTargetProvider, ISharedCarryAnchorPreviewProvider
{
    ...
    public bool TryGetAnchorPreview(PlayerInteractionNew player, out SharedCarryAnchorPreview preview)
    {
        ...
        Vector3? explicitPreviewLocalPoint = TryGetCarryAnchorPreviewLocalPoint(attachPointIndex, out Vector3 configuredPreviewLocalPoint)
            ? configuredPreviewLocalPoint
            : null;
        Vector3 markerWorldPoint = _sharedCarryPhysicsBody.ResolvePreviewGripPointForPose(
            attachLocalPoint,
            explicitPreviewLocalPoint,
            previewPosition,
            previewRotation,
    ...
    {
        previewLocalPoint = Vector3.zero;
        if (mountableBridgeComponentSO == null
            || mountableBridgeComponentSO.carryAttachLocalPoints == null
            || mountableBridgeComponentSO.carryAnchorPreviewLocalPoints == null
            || mountableBridgeComponentSO.carryAnchorPreviewLocalPoints.Length != mountableBridgeComponentSO.carryAttachLocalPoints.Length
            || attachPointIndex < 0
            || attachPointIndex >= mountableBridgeComponentSO.carryAnchorPreviewLocalPoints.Length)
        {
            return false;
        }

        previewLocalPoint = mountableBridgeComponentSO.carryAnchorPreviewLocalPoints[attachPointIndex];
        return true;
    }
