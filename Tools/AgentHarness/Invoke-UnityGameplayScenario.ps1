...
if ([string]::IsNullOrWhiteSpace($ArtifactDirectory)) {
    ...
    $ArtifactDirectory = Join-Path $projectRoot (Join-Path 'Artifacts\Validation\UnityGameplayScenario' $id)
}
...
$reportPath = Join-Path $ArtifactDirectory 'unity-gameplay-scenario-report.json'
$testFilter = 'RageQuitting.Tests.PlayMode.TutorialSceneGameplayScenarioTests.TutorialBoot_LoadsStableSceneWithRequiredGameplayComponents'
$report = [ordered]@{
    schemaVersion = 1
    scenario = $Scenario
    testFilter = $null
...
function Complete-Scenario([string]$Status, [int]$Code, [string[]]$Errors = @()) {
    ...
    if ($Json) {
        ...
        Write-Host "Unity gameplay scenario '$Scenario': $Status (exit $Code)"
        Write-Host "Report: $reportPath"
    }
    exit $Code
}

if ($Scenario -cne 'TutorialBoot') {
    Complete-Scenario 'failed' 1 @("Unknown gameplay scenario '$Scenario'. Available scenarios: TutorialBoot.")
}

$report.testFilter = $testFilter
...
if (-not $execution.available) {
    Complete-Scenario 'failed' 1 @("PlayMode runner could not be started: $($execution.error)")
}
...
try {
    ...
    Complete-Scenario 'failed' 1 @("PlayMode runner output could not be parsed: $($_.Exception.Message)")
}
...
if (-not $envelope.PSObject.Properties['reportPath'] -or
    ...
    Complete-Scenario 'failed' 1 @('PlayMode runner did not return an existing test report.')
}
...
try {
    ...
    Complete-Scenario 'failed' 1 @("PlayMode test report could not be parsed: $($_.Exception.Message)")
}
...
if ($missingReportProperties.Count -gt 0) {
    Complete-Scenario 'failed' 1 @(
        "Underlying PlayMode report is missing required fields: $($missingReportProperties -join ', ').")
}

if ($null -eq $testReport.summary) {
    Complete-Scenario 'failed' 1 @('Underlying PlayMode report does not contain a test summary.')
}
...
if ($null -eq $expectedExitCode) {
    Complete-Scenario 'failed' 1 @("Underlying PlayMode report has unknown status '$underlyingStatus'.")
}

$consistencyErrors = @()
if ([string]$testReport.filter -cne $testFilter) {
    $consistencyErrors += 'Underlying PlayMode report does not contain the exact TutorialBoot test filter.'
}
...
if ([int]$testReport.exitCode -ne [int]$expectedExitCode) {
    $consistencyErrors += "PlayMode report exitCode does not match status '$underlyingStatus'."
}
if ([int]$envelope.exitCode -ne [int]$expectedExitCode) {
    $consistencyErrors += "PlayMode runner envelope exitCode does not match status '$underlyingStatus'."
}
if ([int]$execution.exitCode -ne [int]$expectedExitCode) {
    $consistencyErrors += "PlayMode runner process exitCode does not match status '$underlyingStatus'."
}
if ($consistencyErrors.Count -gt 0) {
    Complete-Scenario 'failed' 1 $consistencyErrors
}
...
if ($underlyingStatus -eq 'passed') {
    ...
    if (-not $hasExactPassingSummary) {
        Complete-Scenario 'failed' 1 @(
            'TutorialBoot requires exactly total=1, passed=1, failed=0, skipped=0 and inconclusive=0 in the underlying PlayMode report.')
    }
}

Complete-Scenario $underlyingStatus ([int]$testReport.exitCode) @($testReport.errors)
